...
# 4 "./tlib/stack.template.h" 2
# 5 "main-stack.c" 2


typedef struct stack_int_node *stack_int_node; typedef struct stack_int { stack_int_node top_ptr; int *top; } *stack_int; stack_int stack_int_newstack(); _Bool stack_int_isEmpty(stack_int st); int stack_int_peek(stack_int st); _Bool stack_int_push(stack_int st, int val); int stack_int_pop(stack_int st); void stack_int_print(stack_int st); void stack_int_free(stack_int *st_ptr);;
struct stack_int_node { int val; stack_int_node next; }; stack_int stack_int_newstack() { stack_int st = ({ struct stack_int *tmp = calloc(1, sizeof(struct stack_int)); if (!tmp) abort(); tmp; }); st->top_ptr = ((void*)0); st->top = ((void*)0); return st; } _Bool stack_int_isEmpty(stack_int st) { if (!st) abort(); return !(st->top_ptr); } int stack_int_peek(stack_int st) { if (!st) abort(); if (st->top_ptr) return st->top_ptr->val; else return (int) 0; } _Bool stack_int_push(stack_int st, int val) { if (!st) abort(); stack_int_node node = ({ struct stack_int_node *tmp = calloc(1, sizeof(struct stack_int_node)); if (!tmp) abort(); tmp; }); node->val = val; node->next = st->top_ptr; st->top_ptr = node; st->top = &st->top_ptr->val; return 1; } int stack_int_pop(stack_int st) { if (!st) abort(); if (!st->top_ptr) return (int) 0; stack_int_node tmp = st->top_ptr; st->top_ptr = tmp->next; st->top = &st->top_ptr->val; int retv = tmp->val; free(tmp); return retv; } void stack_int_print(stack_int st) { if (!st) abort(); stack_int_node p = st->top_ptr; if (!p) { printf("{ }\n"); return; } printf("{\n"); while (p) { const int value = p->val; printf("    "); { const stack_int st = ((void*)0); const stack_int_node p = ((void*)0); (st, p); printf("%d", value); } printf("\n"); p = p->next; } printf("}\n"); } void stack_int_free(stack_int *st_ptr) { if (!st_ptr || !*st_ptr) return; stack_int_node p = (*st_ptr)->top_ptr; while (p) { free(p); p = p->next; } free(*st_ptr); *st_ptr = ((void*)0); };


typedef struct stack_double_node *stack_double_node; typedef struct stack_double { stack_double_node top_ptr; double *top; } *stack_double; stack_double stack_double_newstack(); _Bool stack_double_isEmpty(stack_double st); double stack_double_peek(stack_double st); _Bool stack_double_push(stack_double st, double val); double stack_double_pop(stack_double st); void stack_double_print(stack_double st); void stack_double_free(stack_double *st_ptr);;
struct stack_double_node { double val; stack_double_node next; }; stack_double stack_double_newstack() { stack_double st = ({ struct stack_double *tmp = calloc(1, sizeof(struct stack_double)); if (!tmp) abort(); tmp; }); st->top_ptr = ((void*)0); st->top = ((void*)0); return st; } _Bool stack_double_isEmpty(stack_double st) { if (!st) abort(); return !(st->top_ptr); } double stack_double_peek(stack_double st) { if (!st) abort(); if (st->top_ptr) return st->top_ptr->val; else return (double) 0; } _Bool stack_double_push(stack_double st, double val) { if (!st) abort(); stack_double_node node = ({ struct stack_double_node *tmp = calloc(1, sizeof(struct stack_double_node)); if (!tmp) abort(); tmp; }); node->val = val; node->next = st->top_ptr; st->top_ptr = node; st->top = &st->top_ptr->val; return 1; } double stack_double_pop(stack_double st) { if (!st) abort(); if (!st->top_ptr) return (double) 0; stack_double_node tmp = st->top_ptr; st->top_ptr = tmp->next; st->top = &st->top_ptr->val; double retv = tmp->val; free(tmp); return retv; } void stack_double_print(stack_double st) { if (!st) abort(); stack_double_node p = st->top_ptr; if (!p) { printf("{ }\n"); return; } printf("{\n"); while (p) { const double value = p->val; printf("    "); { const stack_double st = ((void*)0); const stack_double_node p = ((void*)0); (st, p); printf("%.03lf", value); } printf("\n"); p = p->next; } printf("}\n"); } void stack_double_free(stack_double *st_ptr) { if (!st_ptr || !*st_ptr) return; stack_double_node p = (*st_ptr)->top_ptr; while (p) { free(p); p = p->next; } free(*st_ptr); *st_ptr = ((void*)0); };

int main()
{

    stack_double st = stack_double_newstack();
    stack_double_push(st, 388.88);
    stack_double_push(st, 26.90);
    printf("double stack = "); stack_double_print(st);
    stack_double_free(&st);

    stack_int st2 = stack_int_newstack();
    stack_int_push(st2, 12);
    stack_int_push(st2, 46);
    printf("int stack = "); stack_int_print(st2);
    stack_int_free(&st2);
    return 0;
}
