...
typedef struct map_int { int key; int value; struct map_int *prev; struct map_int *next; struct map_int *end;} *map_int;map_int map_int_newmap();map_int map_intfind_key(map_int m, int key);int map_int_get(map_int m, int key, _Bool *found);_Bool map_int_set(map_int m, int key, int val);_Bool map_int_del(map_int m, int key);void map_int_print(map_int m);void map_int_free(map_int *m);map_int map_int_newmap(){ map_int m = ({ struct map_int *tmp = calloc(1, sizeof(struct map_int)); if (!tmp) abort(); tmp;}); m->key = 0; m->value = 0; m->prev = ((void*)0); m->next = ((void*)0); m->end = m; return m;}map_int map_int_find_key(map_int m, int key){ if (!m) abort(); map_int p = m->next; while (p != ((void*)0)) { if (p->key == key) return p; p = p->next; } return ((void*)0);}int map_int_get(map_int m, int key, _Bool *found){ if (!m) abort(); map_int node = map_int_find_key(m, key); if (!node) { *found = 0; return 0; } *found = 1; return node->value;}_Bool map_int_set(map_int m, int key, int val){ if (!m) abort(); map_int node = map_int_find_key(m, key); if (node) { node->value = val; } else { node = ({ struct map_int *tmp = calloc(1, sizeof(struct map_int)); if (!tmp) abort(); tmp;}); node->key = key; node->value = val; node->prev = m->end; node->next = ((void*)0); node->end = ((void*)0); if (m->end) m->end->next = node; else m->next = node; m->end = node; } return 1;}_Bool map_int_del(map_int m, int key){ if (!m) abort(); map_int node = map_int_find_key(m, key); if (!node) return 0; map_int tmp = node; node = node->prev; node->next = tmp->next; tmp->next->prev = node; free(tmp); return 1; }void map_int_print(map_int m){ if (!m) abort(); map_int p = m->next; printf("{\n"); while (p != ((void*)0)) { printf("    %d => " "%d", p->key, p->value); printf("\n"); p = p->next; } printf("}\n");}void map_int_free(map_int *m){ if (!*m) abort(); map_int p = *m; while (p != ((void*)0)) { map_int tmp = p; p = p->next; free(tmp); } *m = ((void*)0);};
